...
tool_subtract_numbers = {
    ...
    "function": {
        ...
        "parameters": {
            ...
            "required": ["x", "y"],
            "properties": {
                "x": {"type": "number", "description": "number to subtract from"},
                "y": {"type": "number", "description": "number to subtract"}
            }
        }
    }
}
...
# Build the request body with tools — include all three so the LLM can pick the right one
body = {
    "model": MODEL,
    "messages": messages,
    "tools": [tool_add_two_numbers, tool_subtract_numbers, tool_multiply_numbers],
    "stream": False
}
